...
- [ ] Host, Server ve Client olarak oyunu başlatmayı sağlayacak temel bir UI (Kullanıcı Arayüzü) oluşturulması.
- [ ] Basit bir test oyuncu (Player) nesnesi oluşturulması.
- [ ] Oyuncu (Player) transform (pozisyon ve rotasyon) senkronizasyonunun sağlanması.

## Düşük Öncelikli Görevler (Low Priority)
- [ ] Geliştirilen sistemin başka bir projeye taşınabilirliğinin test edilmesi.
- [ ] `.unitypackage` olarak ilk dışa aktarmanın (export) yapılması.
